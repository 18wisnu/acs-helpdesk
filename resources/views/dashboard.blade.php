                ...
                <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-50/50">
                    ...
                    <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                        <form action="{{ route('helpdesk.cleanup-discovery') }}" method="POST">
                            @csrf
                                <button type="submit" class="w-full md:w-auto flex justify-center items-center gap-x-2 rounded-lg bg-rose-50 px-4 py-2 sm:py-2.5 text-sm font-bold text-rose-700 border border-rose-200 hover:bg-rose-100 transition-all" onclick="return confirm('Hapus perangkat Discovery (Unknown) & Perangkat lama (>30 hari)?')">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group" x-data="{ openWifiModal: false, openCustomerModal: false }">
                                ...
                                <td class="px-6 py-5 text-right">
                                    <div class="flex justify-end gap-2.5">
                                        ...
                                        <form action="{{ route('helpdesk.provision', $device->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="p-2 text-slate-300 hover:text-emerald-500 transition group" title="Zero Touch Provisioning">
                                                <svg class="h-5 w-5 group-hover:animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                                            </button>
                                        </form>
                                        @endif
